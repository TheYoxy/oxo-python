...
def TestVictoire():
	...
	e=0
	while (e<=2):
		if(grille[e][0] == grille[e][1] and grille[e][1] == grille[e][2]):
			if(grille[e][0] == "x"):
				Victoire("x")
				return 1
			elif(grille[e][0] == "o"):
				Victoire("o")
				return 1
		if(grille[0][e] == grille[1][e] and grille[1][e] == grille[2][e]):
			if(grille[0][e] == "x"):
				Victoire("x")
				return 1
			elif(grille[0][e] == "o"):
	...
	if(grille[0][0] == grille[1][1] and grille[1][1] == grille[2][2]):
		if(grille[0][0] == "x"):
			...
			Victoire("o")
			return 1
	elif(grille[0][2] == grille[1][1] and grille[1][1] == grille[2][0]):
		if(grille[0][2] == "x"):
			Victoire("x")
			return 1
		elif(grille[0][2] == "o"):
...
Pion = Fen.create_oval(PosX-50,PosY-50,PosX+50,PosY+50,width=2)
	#X
Pionx1 = Fen.create_line(PosX-50,PosY-50,PosX+50,PosY+50,width=2,state='hidden')
Pionx2 = Fen.create_line(PosX+50,PosY-50,PosX-50,PosY+50,width=2,state='hidden')
...
Fen.bind('<Return>',Changement)
Fen.pack(padx = 5, pady = 5)


# Création d'un widget Button (bouton Quitter)
Button(Mafenetre, text ='Fin du Game', command = Mafenetre.destroy).pack(side=BOTTOM,padx=5,pady=5)
